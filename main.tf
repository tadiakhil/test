...
resource "aws_s3_bucket" "akhil_cdn_bucket" {
  bucket = "${var.domain}"
  acl = "public-read"
...
resource "aws_route53_record" "root_domain" {
  ...
  alias {
    name = "${aws_cloudfront_distribution.akhil_cdn_distribution.domain_name}"
    zone_id = "${aws_cloudfront_distribution.akhil_cdn_distribution.hosted_zone_id}"
    evaluate_target_health = false
  }
}

resource "aws_cloudfront_distribution" "akhil_cdn_distribution" {
  origin {
...
output "cdn_domain" {
  value = "${aws_cloudfront_distribution.akhil_cdn_distribution.domain_name}"
}